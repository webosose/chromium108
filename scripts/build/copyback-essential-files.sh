...
cp -r $origin_dir/src/tools/android/. src/tools/android

mkdir -p src/third_party/swiftshader/tests/SystemUnitTests
cp $origin_dir/src/third_party/swiftshader/tests/SystemUnitTests/BUILD.gn src/third_party/swiftshader/tests/SystemUnitTests

mkdir -p src/chrome/test/data/cast
cp $origin_dir/src/chrome/test/data/cast/BUILD.gn src/chrome/test/data/cast

mkdir -p src/chrome/test/data/pdf
cp $origin_dir/src/chrome/test/data/pdf/BUILD.gn src/chrome/test/data/pdf

mkdir -p src/chrome/test/data/webui
cp -r $origin_dir/src/chrome/test/data/webui/. src/chrome/test/data/webui

mkdir -p src/third_party/swiftshader/tests
cp -r $origin_dir/src/third_party/swiftshader/tests/. src/third_party/swiftshader/tests

mkdir -p src/ios
cp $origin_dir/src/ios/features.gni src/ios

mkdir -p src/content/test/data
cp $origin_dir/src/content/test/data/web_ui_* src/content/test/data

mkdir -p src/chromecast
cp $origin_dir/src/chromecast/chromecast.gni src/chromecast

#Chromium91 more
mkdir -p src/third_party/catapult/third_party
cp -r $origin_dir/src/third_party/catapult/third_party/. src/third_party/catapult/third_party

#Chromium108 more
cp $origin_dir/src/tools/perf/process_perf_results.pydeps src/tools/perf

mkdir -p src/chrome/test/data/extensions
cp -r $origin_dir/src/chrome/test/data/extensions/. src/chrome/test/data/extensions

rm src/third_party/swiftshader/tests/regres/testlists/vk-master-NOT_SUPPORTED.txt
