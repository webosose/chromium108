...
void WaylandSeatManager::AddSeat(WaylandConnection* connection,
  ...
  seat_list_.push_back(
      std::make_unique<neva::WaylandSeat>(connection, seat_id, seat));
}

neva::WaylandSeat* WaylandSeatManager::GetFirstSeat() const {
  if (!seat_list_.empty())
...
void WaylandSeatManager::SetCursorPlatformShape(wl_cursor* cursor_data,
  ...
  for (auto& seat : seat_list_)
    if (seat->cursor())
      seat->cursor()->SetPlatformShape(cursor_data, buffer_scale);
}
