...
#define PMTRACE_LOG(label) pmtracer::PmTraceTraceMessage(label)

// PMTRACE_ITEM is for tracing (name, value) pairs.
// Both name and value are strings.
#define PMTRACE_ITEM(name, value) pmtracer::PmTraceTraceItem(name, value)
...
#define PMTRACE_BEFORE(label) pmtracer::PmTraceTraceBefore(label)
#define PMTRACE_AFTER(label) pmtracer::PmTraceTraceAfter(label)
...
#define PMTRACE_SCOPE_ENTRY(label) pmtracer::PmTraceTraceScopeEntry(label)
#define PMTRACE_SCOPE_EXIT(label) pmtracer::PmTraceTraceScopeExit(label)
#define PMTRACE_SCOPE(label) PmTraceTraceScope traceScope(label)

// PMTRACE_FUNCTION* is for tracing a the duration of a scope.
// In C++ code use PMTRACE_FUNCTION only, in C code use the
// ENTRY/EXIT macros and be careful to catch all exit cases.
#define PMTRACE_FUNCTION_ENTRY(label) pmtracer::PmTraceTraceFunctionEntry(label)
#define PMTRACE_FUNCTION_EXIT(label) pmtracer::PmTraceTraceFunctionExit(label)
...
class PmTraceTraceScope {
 public:
  PmTraceTraceScope(char* label) : scopeLabel(label) {
...
class PmTraceTraceFunction {
 public:
  PmTraceTraceFunction(char* label) : fnLabel(label) {
