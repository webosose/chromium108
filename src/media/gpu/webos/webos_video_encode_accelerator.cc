...
uint8_t WebOSVideoEncodeAccelerator::GetH264LevelLimit(
  ...
  VLOG(2) << __func__;
  int kH264MacroblockSizeInPixels = 16;
  const uint32_t mb_width =
      base::bits::AlignUp(config->width, kH264MacroblockSizeInPixels) /
      kH264MacroblockSizeInPixels;
  const uint32_t mb_height =
      base::bits::AlignUp(config->height, kH264MacroblockSizeInPixels) /
      kH264MacroblockSizeInPixels;
...
void WebOSVideoEncodeAccelerator::RequestEncodingParametersChangeTask(
  ...
  if (current_bitrate_ == bitrate.target_bps() && current_framerate_ == framerate)
    return;
  ...
  VLOG(2) << __func__ << " bitrate=" << bitrate.ToString()
          << ", framerate=" << framerate;
  if (video_encoder_api_->UpdateEncodingParams(bitrate.target_bps(), framerate)) {
