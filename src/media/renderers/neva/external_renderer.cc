...
ExternalRenderer::ExternalRenderer(
  ...
  NEVA_DCHECK(create_video_decoders_cb_);
  VLOG(1) << __func__;

  video_sink_.reset(
      new NullVideoSink(false, base::Seconds(1.0 / 30),
                        base::BindRepeating(&ExternalRenderer::FrameReady,
...
void ExternalRenderer::Flush(base::OnceClosure flush_cb) {
  ...
  if (buffering_state_ != BUFFERING_HAVE_NOTHING) {
    ...
    task_runner_->PostTask(
        FROM_HERE, base::BindOnce(&ExternalRenderer::OnBufferingStateChange,
...
void ExternalRenderer::StartPlayingFrom(base::TimeDelta timestamp) {
  ...
  NEVA_DCHECK(task_runner_->BelongsToCurrentThread());
  NEVA_DCHECK(ready_frames_.empty());
...
scoped_refptr<VideoFrame> ExternalRenderer::Render(
  ...
  NEVA_DCHECK(task_runner_->BelongsToCurrentThread());
  TRACE_EVENT0("media", "ExternalRenderer::Render");
  ...
  if (!ready_frames_.empty()) {
    ...
    task_runner_->PostTask(
        FROM_HERE,
        base::BindOnce(&ExternalRenderer::AttemptReadAndCheckForMetadataChanges,
...
void ExternalRenderer::InitializeMediaPlatformAPI() {
  NEVA_DCHECK(task_runner_->BelongsToCurrentThread());
  NEVA_DCHECK(media_platform_api_);
  ...
  media_platform_api_->Initialize(
      ...
      base::BindRepeating(&ExternalRenderer::OnMediaPlatformAPIInitialized,
...
void ExternalRenderer::InitializeAudioDecoder() {
  ...
  if (audio_stream) {
    ...
    NEVA_DCHECK(current_audio_decoder_config_.IsValidConfig());

    audio_decoder_stream_ = std::make_unique<AudioDecoderStream>(
        std::make_unique<AudioDecoderStream::StreamTraits>(
            media_log_, CHANNEL_LAYOUT_NONE,
            current_audio_decoder_config_.target_output_sample_format()),
    ...
    audio_decoder_stream_->Initialize(
        audio_stream,
        base::BindRepeating(&ExternalRenderer::OnAudioDecoderStreamInitialized,
                            weak_factory_.GetWeakPtr()),
        cdm_context_,
        base::BindRepeating(&ExternalRenderer::OnStatisticsUpdate,
                            weak_factory_.GetWeakPtr()),
        base::BindRepeating(&ExternalRenderer::OnWaiting,
                            weak_factory_.GetWeakPtr()));
    has_audio_ = true;
  } else {
...
void ExternalRenderer::InitializeVideoDecoder() {
  ...
  if (video_stream) {
    ...
    NEVA_DCHECK(current_video_decoder_config_.IsValidConfig());

    video_decoder_stream_->Initialize(
        video_stream,
        base::BindRepeating(&ExternalRenderer::OnVideoDecoderStreamInitialized,
                            weak_factory_.GetWeakPtr()),
        cdm_context_,
        base::BindRepeating(&ExternalRenderer::OnStatisticsUpdate,
                            weak_factory_.GetWeakPtr()),
...
void ExternalRenderer::OnAudioConfigChange(const AudioDecoderConfig& config) {
  NEVA_DCHECK(task_runner_->BelongsToCurrentThread());
  NEVA_DCHECK(config.IsValidConfig());
  DVLOG(1) << __func__;
  if (client_)
...
void ExternalRenderer::AudioBufferReady(AudioDecoderStream::ReadResult result) {
  NEVA_DCHECK(task_runner_->BelongsToCurrentThread());
  NEVA_DCHECK(pending_audio_read_);
  pending_audio_read_ = false;
  ...
  if (result.has_error()) {
    task_runner_->PostTask(
        FROM_HERE,
        base::BindOnce(&ExternalRenderer::OnPlaybackError,
                       weak_factory_.GetWeakPtr(), PIPELINE_ERROR_DECODE));
    return;
  }

  NEVA_DCHECK(result.has_value());
  scoped_refptr<AudioBuffer> buffer = std::move(result).value();
  NEVA_DCHECK(buffer);

  if (buffer->end_of_stream()) {
    NEVA_DCHECK(!audio_received_end_of_stream_);
...
void ExternalRenderer::VideoFrameReady(VideoDecoderStream::ReadResult result) {
  NEVA_DCHECK(task_runner_->BelongsToCurrentThread());
  NEVA_DCHECK(pending_video_read_);
  pending_video_read_ = false;
  ...
  if (is_eos) {
    NEVA_DCHECK(!video_received_end_of_stream_);
...
void ExternalRenderer::AddReadyFrame(const scoped_refptr<VideoFrame> frame) {
  NEVA_DCHECK(task_runner_->BelongsToCurrentThread());
  NEVA_DCHECK(!frame->metadata().end_of_stream);
...
void ExternalRenderer::AttemptRead() {
  ...
  switch (render_state_) {
    case RenderState::kPlaying:
      ...
      if (has_audio_ && !pending_audio_read_ && !ReceivedEOSByType(Audio) &&
          media_platform_api_->AllowedFeedAudio()) {
        pending_audio_read_ = true;
        audio_decoder_stream_->Read(
            base::BindOnce(&ExternalRenderer::AudioBufferReady,
                           frame_callback_weak_factory_.GetWeakPtr()));
      }
      if (has_video_ && !pending_video_read_ && !ReceivedEOSByType(Video) &&
          media_platform_api_->AllowedFeedVideo()) {
        pending_video_read_ = true;
...
void ExternalRenderer::FlushAudioDecoder() {
  if (has_audio_) {
    audio_decoder_stream_->Reset(
        base::BindOnce(&ExternalRenderer::OnAudioDecoderStreamResetDone,
                       weak_factory_.GetWeakPtr()));
  } else {
...
void ExternalRenderer::FlushVideoDecoder() {
  if (has_video_) {
    video_decoder_stream_->Reset(
        base::BindOnce(&ExternalRenderer::OnVideoDecoderStreamResetDone,
                       weak_factory_.GetWeakPtr()));
  } else {
...
void ExternalRenderer::OnAudioDecoderStreamResetDone() {
  NEVA_DCHECK(task_runner_->BelongsToCurrentThread());
  NEVA_DCHECK(!sink_started_);
  DCHECK_EQ(RenderState::kFlushing, render_state_);
  NEVA_DCHECK(!audio_received_end_of_stream_);
  NEVA_DCHECK(!rendered_end_of_stream_);
  DCHECK_EQ(buffering_state_, BUFFERING_HAVE_NOTHING);
...
void ExternalRenderer::MaybeFireEndedCallback() {
  ...
  if (ReceivedEndOfStream() && !rendered_end_of_stream_) {
    ...
    task_runner_->PostTask(FROM_HERE,
                           base::BindOnce(&ExternalRenderer::OnPlaybackEnded,
                                          weak_factory_.GetWeakPtr()));
  }
}
...
void ExternalRenderer::SetMediaPlatformAPICb() {
  ...
  if (media_platform_api_) {
    media_platform_api_->SetPlayerEventCb(BindToCurrentLoop(base::BindRepeating(
        &ExternalRenderer::OnPlayerEvent, weak_factory_.GetWeakPtr())));

    media_platform_api_->SetStatisticsCb(base::BindRepeating(
...
void ExternalRenderer::OnPlayerEvent(PlayerEvent event) {
  ...
  if (event == PlayerEvent::kSuspendDone) {
    Flush(base::BindOnce(&ExternalRenderer::OnFlushDone,
                         weak_factory_.GetWeakPtr()));
  }
