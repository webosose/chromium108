...
std::unique_ptr<Renderer> NevaMediaPlayerRendererFactory::CreateRenderer(
    ...
    const gfx::ColorSpace& target_color_space) {
  NEVA_DCHECK(audio_renderer_sink);
  NEVA_DCHECK(media_platform_api_) << "WebOS Media API uninitialized";
  ...
  std::unique_ptr<ExternalRenderer> renderer(new ExternalRenderer(
      media_task_runner, media_platform_api_,
      base::BindRepeating(&NevaMediaPlayerRendererFactory::CreateAudioDecoders,
                          base::Unretained(this), media_task_runner),
      base::BindRepeating(&NevaMediaPlayerRendererFactory::CreateVideoDecoders,
                          base::Unretained(this), media_task_runner,
