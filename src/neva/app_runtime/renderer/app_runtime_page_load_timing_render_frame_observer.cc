...
bool AppRuntimePageLoadTimingRenderFrameObserver::
  ...
  if ((perf.LargestTextPaintForMetrics() < 0.0) &&
      (perf.LargestImagePaintForMetrics() < 0.0))
    return false;

  if ((perf.LargestTextPaintSizeForMetrics() <
       largest_contentful_paint_size_) &&
      (perf.LargestImagePaintSizeForMetrics() < largest_contentful_paint_size_))
    return false;

  if (perf.LargestTextPaintSizeForMetrics() >
      perf.LargestImagePaintSizeForMetrics()) {
    largest_contentful_paint_ =
        ClampDelta(perf.LargestTextPaintForMetrics(), perf.NavigationStart());
    largest_contentful_paint_size_ = perf.LargestTextPaintSizeForMetrics();
  } else {
    largest_contentful_paint_ =
        ClampDelta(perf.LargestImagePaintForMetrics(), perf.NavigationStart());
    largest_contentful_paint_size_ = perf.LargestImagePaintSizeForMetrics();
  }
  return true;
}
