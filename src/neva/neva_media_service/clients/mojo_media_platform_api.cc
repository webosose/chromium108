...
MojoMediaPlatformAPI::MojoMediaPlatformAPI(
  ...
  media_task_runner_->PostTask(
      FROM_HERE, base::BindOnce(&MojoMediaPlatformAPI::OnCreate, weak_this_));
}
...
void MojoMediaPlatformAPI::OnCreate() {
  ...
  provider->CreateMediaPlatformAPI(
      ...
      media_platform_api_.BindNewPipeAndPassReceiver());
}
...
void MojoMediaPlatformAPI::Initialize(
  ...
  if (media_platform_api_) {
    ...
    absl::optional<media::AudioDecoderConfig> maybe_audio_config;
    absl::optional<media::VideoDecoderConfig> maybe_video_config;
    ...
    media_platform_api_->Initialize(
        ...
        base::BindOnce(&MojoMediaPlatformAPI::OnInitialized, weak_this_));
  }
}
...
void MojoMediaPlatformAPI::OnInitialized(const media::PipelineStatus& status) {
  NEVA_DCHECK(media_task_runner_->BelongsToCurrentThread());
...
void MojoMediaPlatformAPI::Suspend(media::SuspendReason reason) {
  if (!media_task_runner_->BelongsToCurrentThread()) {
    media_task_runner_->PostTask(
        FROM_HERE,
        base::BindOnce(&MojoMediaPlatformAPI::Suspend, weak_this_, reason));
    return;
  }
...
void MojoMediaPlatformAPI::Resume(
  ...
  if (!media_task_runner_->BelongsToCurrentThread()) {
    media_task_runner_->PostTask(
        FROM_HERE, base::BindOnce(&MojoMediaPlatformAPI::Resume, weak_this_,
...
void MojoMediaPlatformAPI::Finalize() {
  if (!media_task_runner_->BelongsToCurrentThread()) {
    media_task_runner_->PostTask(
        FROM_HERE, base::BindOnce(&MojoMediaPlatformAPI::Finalize, weak_this_));
    return;
  }
...
void MojoMediaPlatformAPI::SetKeySystem(const std::string& key_system) {
  if (!media_task_runner_->BelongsToCurrentThread()) {
    media_task_runner_->PostTask(
        FROM_HERE, base::BindOnce(&MojoMediaPlatformAPI::SetKeySystem,
                                  weak_this_, key_system));
    return;
  }
...
void MojoMediaPlatformAPI::SetDisableAudio(bool disable) {
  if (!media_task_runner_->BelongsToCurrentThread()) {
    media_task_runner_->PostTask(
        FROM_HERE, base::BindOnce(&MojoMediaPlatformAPI::SetDisableAudio,
                                  weak_this_, disable));
    return;
  }
...
void MojoMediaPlatformAPI::SetMediaLayerId(const std::string& media_layer_id) {
  if (!media_task_runner_->BelongsToCurrentThread()) {
    media_task_runner_->PostTask(
        FROM_HERE, base::BindOnce(&MojoMediaPlatformAPI::SetMediaLayerId,
...
void MojoMediaPlatformAPI::SetMediaPreferences(const std::string& preferences) {
  if (!media_task_runner_->BelongsToCurrentThread()) {
    media_task_runner_->PostTask(
        FROM_HERE, base::BindOnce(&MojoMediaPlatformAPI::SetMediaPreferences,
...
void MojoMediaPlatformAPI::SetMediaCodecCapabilities(
  ...
  if (!media_task_runner_->BelongsToCurrentThread()) {
    media_task_runner_->PostTask(
        FROM_HERE,
        base::BindOnce(&MojoMediaPlatformAPI::SetMediaCodecCapabilities,
