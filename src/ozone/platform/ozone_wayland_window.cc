...
void OzoneWaylandWindow::SetCustomCursor(neva_app_runtime::CustomCursorType type,
  ...
  if (type == neva_app_runtime::CustomCursorType::kPath) {
    base::ThreadTaskRunnerHandle::Get()->PostTask(
        FROM_HERE,
        base::BindOnce(&CreateBitmapFromPng, type, path, hotspot_x, hotspot_y,
                       allowed_cursor_overriding,
                       base::BindOnce(
                          &OzoneWaylandWindow::SetCustomCursorFromBitmap,
...
void OzoneWaylandWindow::SetCustomCursorFromBitmap(
  ...
  scoped_refptr<PlatformCursor> cursor = CursorFactory::GetInstance()->CreateImageCursor(
      mojom::CursorType::kPointer, *cursor_image, gfx::Point(hotspot_x, hotspot_y));
