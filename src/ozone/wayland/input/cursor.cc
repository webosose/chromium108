...
WaylandCursor::WaylandCursor()
    : input_pointer_(NULL), buffer_(NULL), width_(0), height_(0) {
...
#if defined(OS_WEBOS)
    if (location == ui::lsm_cursor_hide_hotspot ||
        location == ui::lsm_cursor_restore_hotspot)
      SetLSMCursorAndCommit(serial, location);
    else
#endif
      HideCursor(serial);
...
#if defined(OS_WEBOS)
void WaylandCursor::SetLSMCursorAndCommit(uint32_t serial,
