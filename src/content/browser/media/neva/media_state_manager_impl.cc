...
void MediaStateManagerImpl::OnWebContentsDestroyed(WebContents* web_contents) {
  web_contents->ForEachRenderFrameHost(
      [this](content::RenderFrameHost* rfh) { OnRenderFrameDeleted(rfh); });
}

void MediaStateManagerImpl::ResumeAllMedia(WebContents* web_contents) {
  web_contents->ForEachRenderFrameHost(
      [](content::RenderFrameHost* frame) { frame->SetSuppressed(false); });
...
void MediaStateManagerImpl::SuspendAllMedia(WebContents* web_contents) {
  web_contents->ForEachRenderFrameHost(
      [](content::RenderFrameHost* frame) { frame->SetSuppressed(true); });
...
void MediaStateManagerImpl::PermitMediaActivation(
  ...
  media_observer
      ->GetMediaPlayerRemote(
          MediaPlayerId(render_frame_host->GetGlobalId(), player_id))
      ->RequestActivation();
}
